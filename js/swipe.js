var SwipeApp = {
    ...
    init: async function() {
        ...
        try {
            ...
            // ✅ Проверяем завершённость профиля
            if (!meData.profile_complete) {
                window.location.href = 'register.html';
                return;
            }

            this.likesRemaining = this.currentUser.premium
                ? Infinity
                : (CONFIG.FREE_LIKES_PER_DAY - (this.currentUser.likes_today || 0));
            this.updateLikesCounter();

        } catch(e) {
            console.error('[SWIPE] getMe failed', e);
            window.location.href = 'index.html';
            return;
        }
    ...
    loadUsers: async function() {
        ...
        try {
            ...
            console.error('[SWIPE] loadUsers failed', e);
            this.showEmpty();
        }
    ...
    renderCards: function() {
        ...
        if (topCard) this.bindDrag(topCard);
    ...
    createCard: function(user, stackIndex) {
        ...
        var lookingHtml = lookingFor
            ? '<div class="card-looking">' + lookingFor + '</div>'
            : '';
    ...
    dragMove: function(e, card) {
        ...
        if (likeBadge) likeBadge.style.opacity = this.currentX > threshold
            ? Math.min((this.currentX - threshold) / 50, 1) : 0;
        if (nopeBadge) nopeBadge.style.opacity = this.currentX < -threshold
            ? Math.min((-this.currentX - threshold) / 50, 1) : 0;
    },
    ...
    doLike: async function(userId) {
        ...
        try {
            ...
            // ✅ Показываем матч только если он НОВЫЙ
            if (res.match === true && res.match_user) {
    ...
    doSkip: async function(userId) {
        TG.haptic('light');
        try { await API.skip(userId); } catch(e) { console.error('[SKIP]', e); }
    },

    likeBtn: function() {
        var container = document.getElementById('cards-container');
        var topCard = container ? container.lastElementChild : null;
        if (topCard && !this.isAnimating) this.animateOut(topCard, 'right');
    },

    skipBtn: function() {
        var container = document.getElementById('cards-container');
        var topCard = container ? container.lastElementChild : null;
        if (topCard && !this.isAnimating) this.animateOut(topCard, 'left');
    },
    ...
    showMatchModal: function(user) {
        ...
        if (photo) {
            var src = (user.photos && user.photos[0])
                ? user.photos[0]
