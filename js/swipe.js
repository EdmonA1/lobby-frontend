var SwipeApp = {
  ...
  createCard: function(user, stackIndex) {
    ...
    var lookingFor = (user.looking_for || []).map(function(l) { return lookingMap[l] || l; }).join(' · ');
  ...
  dragMove: function(e, card) {
    ...
    if (likeBadge) likeBadge.style.opacity = this.currentX > threshold ? Math.min((this.currentX - threshold) / 50, 1) : 0;
    if (nopeBadge) nopeBadge.style.opacity = this.currentX < -threshold ? Math.min((-this.currentX - threshold) / 50, 1) : 0;
  },
  ...
  doLike: async function(userId) {
    ...
    try {
      ...
      if (res.likes_remaining !== undefined) this.likesRemaining = res.likes_remaining;
      else this.likesRemaining = Math.max(0, this.likesRemaining - 1);
      this.updateLikesCounter();
